...
beehive.scene.Game.prototype.initBackground = function() {
    // this.background = new rune.display.Graphic(0, 0, 400, 225, "background");
    // this.stage.addChild(this.background);
...
beehive.scene.Game.prototype.initPlayers = function() {
    // Anta att Player-konstruktörerna är definierade korrekt någon annanstans
    this.player1 = new beehive.Player(9, 8, "bee", this.controller1, this.bullets1, this.honeycombs2, this.honeycombs1);
    this.stage.addChild(this.player1);
    ...
    this.player1.healthBar.x = 75; // Exempelpositioner
    this.player1.healthBar.y = 5;
    this.player2.healthBar.x = 300;
    this.player2.healthBar.y = 5;
};

beehive.scene.Game.prototype.initTimers = function() {
    this.timers.create({
        duration: 10000,
        onTick: this.spawnBird.bind(this),
        scope: this,
        repeat: Infinity
    });
    ...
    this.timers.create({
        duration: 15000,
        onTick: this.spawnBeekeeper.bind(this),
        scope: this,
        repeat: Infinity
    });
    ...
    this.timers.create({
        duration: 20000,
        onTick: this.spawnPowerup.bind(this),
        scope: this,
        repeat: Infinity
    });
};

// beehive.scene.Game.prototype.spawnBird = function() {
//     var bird = new beehive.Bird(Math.random() * (this.stage.width - 38), -24);
//     this.birds.push(bird);
//     this.stage.addChild(bird);
//     console.log("Bird spawned at:", bird.x, bird.y); 
// };

beehive.scene.Game.prototype.spawnBird = function() {
    var bird = new beehive.Bird();
    this.birds.push(bird);
    this.stage.addChild(bird);
};
...
// beehive.scene.Game.prototype.shieldPowerup = function (player) {
//     var shieldSprite = new rune.display.Sprite(0, 0, 13, 172, "bush");
...
//     // Aktivera sköldlogiken i spelaren
//     player.activateShield();

//     // Hantera sköldens sprite och rensa den från scenen efter dess livstid
//     var self = this;
//     this.timers.create({
//         duration: 10000,
//         onTick: function () {
//             self.stage.removeChild(shieldSprite);
//             player.deactivateShield();  // Säkerställ att sköldstatus stängs av när spriten försvinner
//             var index = self.shields.indexOf(shieldSprite);
//             if (index !== -1) {
//                 self.shields.splice(index, 1);
//             }
//         },
...
beehive.scene.Game.prototype.shieldPowerup = function (player) {
    ...
    shieldSprite.visible = true;  // Se till att skölden är synlig
...
beehive.scene.Game.prototype.update = function(step) {
    rune.scene.Scene.prototype.update.call(this, step);
    this.birds.forEach(bird => bird.update(this));
    // this.birds.forEach(function(bird) { bird.update(); });
    this.beekeepers.forEach(function(beekeeper) { beekeeper.update(); });
    ...
    for (var j = 0; j < this.player2.bullets.length; j++) {
        ...
        this.player2.bullets[j].hitTestObject(this.player1, function () {
            ...
            //     this.player1.health -= 2;
            // }
            if (this.player2.isDoubleDamage) { // Antag att isDoubleDamage är det korrekta namnet för att spåra dubbel skada
                this.player1.health -= 4;
            } else {
                this.player1.health -= 2;
            }
    ...
    for (var i = 0; i < this.player1.bullets.length; i++) {
        this.player1.bullets[i].hitTestObject(this.player2, function () {
            ...
            //     this.player2.health -= 2;
            // }
            if (this.player1.isDoubleDamage) { // Antag att isDoubleDamage är det korrekta namnet för att spåra dubbel skada
                this.player2.health -= 4;
            } else {
                this.player2.health -= 2;
            }
...
beehive.scene.Game.prototype.spawnBeekeeper = function() {
    var xPositions = [10, 355]; // Starting X positions for left and right
    var x = xPositions[Math.floor(Math.random() * xPositions.length)];
    var beekeeper = new beehive.Beekeeper(x, -40);
    this.beekeepers.push(beekeeper);
    this.stage.addChild(beekeeper);
};

beehive.scene.Game.prototype.spawnPowerup = function() {
    var types = ["healthTimes2", "shield", "slowEnemyShots"];
    var type = types[Math.floor(Math.random() * types.length)];
    var powerup = new beehive.Powerup(Math.random() * 350, Math.random() * 200, type);
    this.powerups.push(powerup);
    this.stage.addChild(powerup);
...
beehive.scene.Game.prototype.checkPowerupCollisions = function() {
    this.powerups.forEach((powerup, index) => {
        if (this.player1.hitTestObject(powerup)) {
            powerup.activate(this.player1);
            this.stage.removeChild(powerup);
            this.powerups.splice(index, 1);
        } else if (this.player2 && this.player2.hitTestObject(powerup)) {
            powerup.activate(this.player2);
            this.stage.removeChild(powerup);
            this.powerups.splice(index, 1);
        }
    });
